...
if(!file_exists(JPATH_ROOT .DS. 'components' .DS. 'com_k2'.DS.'k2.php')) {
	return;
}
...
abstract class modTCVNK2ScrollerHelper 
{
	...
	public static function getItems(&$params, $format = 'html')
	{
		...
		$limit = $params->get('numberItems', 5);
		...
		if (count($items))
		{
			...
			foreach ($items as $item)
			{
				...
				$postImage = modTCVNK2ScrollerHelper::generateImgWithoutLink($item);
				if (isset($item->$image))
					$item->image = $item->$image;
				else if(isset($postImage->src)) {
					if(strpos($postImage->src,"http://") !== false) {
						$item->image = $postImage->src;
					}
					else {
						if(is_file($postImage->src)) {
							$item->image = JURI::base().$postImage->src;
						} else {
							$item->image = JURI::base().'modules/mod_tcvn_k2_scroller/assets/images/no-image-news.gif';
						}
					}
				}
				else {
	...
	protected static function generateImgWithoutLink($row)
	{	
		if(preg_match('/<img(.*?)>/i', $row->introtext . $row->fulltext, $match))
		{
			$image = $match [0];
			$item  = $match [1];
			$img   = new stdClass();
	...
	public static function loadMediaFiles()
	{
		$document 	= &JFactory::getDocument();
		$url_load   = JURI::base().'modules/mod_tcvn_k2_scroller/assets';
		$document->addStyleSheet($url_load.'/css/style.css'); 
		$document->addScript($url_load.'/js/jquery.cssAnimate.mini.js');
		$document->addScript($url_load.'/js/jquery.mousewheel.min.js');
		$document->addScript($url_load.'/js/jquery.touchwipe.min.js');
		$document->addScript($url_load.'/js/scroller.min.js');
	}
}